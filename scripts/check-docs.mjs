...
import { requireMainSource } from './main-source.mjs';
import assert from 'node:assert/strict';
import { exampleArchives, generateExamples } from './generate-examples.mjs';

const website = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const repository = requireMainSource();
const localSourcePrefixes = ['src/', 'examples/', 'fixtures/'];
const isLocalSource = path => localSourcePrefixes.some(prefix => path.startsWith(prefix));
const sourcePath = path => isLocalSource(path)
  ? resolve(website, path)
...
export function validate(manifest, files, sources, assets = {}) {
  ...
  for (const page of manifest.pages ?? []) {
    ...
    for (const source of page.sources ?? []) {
      ...
      if (source.startsWith('examples/') && source.endsWith('.hs') && /\{\-#\s*LANGUAGE\b/.test(sources[source] ?? '')) errors.push(`Language pragma in tutorial source: ${source}`);
      if (source.startsWith('examples/') && source.endsWith('.hs')) errors.push(...validateCanonicalMarkers(source, sources[source] ?? ''));
    }
...
function selfTest() {
  ...
  const tutorialSource = 'examples/example.hs';
...
if (process.argv.includes('--self-test')) selfTest();
  ...
  try {
    ...
    for (const path of Object.keys(manifest.sourceHashes ?? {})) {
      const absolute = sourcePath(path);
      const sourceBase = isLocalSource(path) ? website : repository;
      if (relative(sourceBase, absolute).startsWith('..') || path.startsWith('/')) throw new Error(`Source must be repository-local: ${path}`);
